...
export default function AdminChangePasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => router.push("/admin-fs9x7k"), 1000);
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4"
      ...
      <div className="w-full max-w-md bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-8">
        ...
        <h1 className="text-xl font-bold text-white text-center mb-1">Change Password</h1>
        <p className="text-sm text-white/50 text-center mb-6">Update your admin account password</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-white/50 uppercase tracking-wide">Current Password</label>
            <input type="password" name="currentPassword" value={form.currentPassword} onChange={handleChange} required placeholder="••••••••"
              className="w-full px-3 py-2 text-sm bg-white/10 border border-white/20 text-white rounded-lg outline-none focus:ring-2 focus:ring-blue-400 transition placeholder:text-white/30" />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-white/50 uppercase tracking-wide">New Password</label>
            <input type="password" name="newPassword" value={form.newPassword} onChange={handleChange} required placeholder="••••••••"
              className="w-full px-3 py-2 text-sm bg-white/10 border border-white/20 text-white rounded-lg outline-none focus:ring-2 focus:ring-blue-400 transition placeholder:text-white/30" />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-white/50 uppercase tracking-wide">Confirm New Password</label>
        ...
        <button type="button" onClick={() => router.push("/admin-fs9x7k")}
          className="w-full mt-4 py-2 text-sm font-medium text-white/50 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg transition">
          ← Back to Admin Login
        </button>
      </div>
    </div>
